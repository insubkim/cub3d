...
/*   Updated: 2023/07/28 21:24:12 by heson            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	init_vars_for_raycasting(t_ray_data *ray, t_player player, double camera_x)
{
	...
	init_side_data_of_ray(&(ray->x), ray->loc.x, ray->dir.x, player.loc.x, ray->dir.y);
	init_side_data_of_ray(&(ray->y), ray->loc.y, ray->dir.y, player.loc.y, ray->dir.x);
...
double	get_dist_of_ray(int x, t_ray_data *ray, t_player player, char **map_board)
{
	...
	while (!is_hit)
	{
		if(ray->x.side_dist < ray->y.side_dist)
			...
			jump_to_next_side(WE, &(ray->y), &(ray->loc.y), &(ray->side)); 
		if(map_board[ray->loc.y][ray->loc.x] == WALL) is_hit = TRUE;
	}
	if(ray->side == NS) 
